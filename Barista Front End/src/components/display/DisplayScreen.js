  ...
  const content = (
    <div className={`h-screen w-full ${hasBg ? '' : theme.bg} ${theme.text}
      ...
      <header className="px-6 md:px-10 pt-5 pb-5 flex items-center justify-between gap-4 shadow-md relative"
        ...
        {/* Scan-to-order QR, always on the board.

            Steve's boss: someone should be able to order from their own
            phone without waiting for the tablet, even while another person
            is mid-order on it. So this stays up in fullscreen too - unlike
            the operator chrome below - and is generated by /api/qr on our
            own origin, because the external QR service fails on venue wifi.

            Hidden when kiosk ordering is the only route the operator wants,
            or when there is no URL to encode. */}
        {orderQrUrl && (
          <div className="flex items-center gap-3 flex-shrink-0 mr-2"
            ...
            <div className="bg-white rounded-xl p-1.5 shadow-sm">
              <img
                // size here is the existing endpoint's BOX size (px per module),
                // not the pixel width - CSS below controls how big it draws.
                src={`/api/qr?size=8&data=${encodeURIComponent(orderQrUrl)}`}
                alt="Scan to order"
                className={isPortrait ? 'w-20 h-20' : 'w-24 h-24'}
                // A QR that fails to load should leave a clean header, not
                // a broken-image box on a customer-facing board.
