...
const EnhancedLiveOperationsDashboard = () => {
  ...
  const getAlertBgColor = (severity) => {
    const colors = {
      critical: 'bg-cq-alert-wash border-cq-alert',
      warning: 'bg-cq-warn-wash border-cq-warn',
      info: 'bg-cq-caramel-wash border-cq-line'
    };
    return colors[severity] || 'bg-cq-wash border-cq-line';
  };
  ...
  const getStationCardStyle = (station) => {
    ...
    if (station.status !== 'active') return 'bg-cq-wash border-cq-line';
    if (loadRatio > 1.2) return 'bg-cq-alert-wash border-cq-alert';
    if (loadRatio > 0.8) return 'bg-cq-warn-wash border-cq-warn';
    if (loadRatio > 0.5) return 'bg-cq-caramel-wash border-cq-line';
    return 'bg-cq-ready-wash border-cq-ready';
  };

  return (
    <div className="p-6 space-y-6 bg-cq-wash min-h-screen">
      {/* Header with Live Status */}
      <div className="bg-cq-milk rounded-cq-lg shadow-cq-card p-4">
        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-3">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
            <h1 className="text-lg sm:text-2xl font-bold text-cq-roast">Live Operations Command Center</h1>
            <div className="flex items-center space-x-2">
              {ordersOnline ? (
                <>
                  <Wifi className="text-cq-ready" size={20} />
                  <span className="text-sm font-medium text-cq-ready">System Online</span>
                </>
              ) : (
                <>
                  <WifiOff className="text-cq-alert" size={20} />
                  <span className="text-sm font-medium text-cq-alert">Offline Mode</span>
                </>
              )}
            </div>
          ...
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 text-cq-ink-2 hover:text-cq-roast"
          >
            <RefreshCw size={16} />
            <span>Refresh</span>
          </button>
      ...
      {criticalAlerts.length > 0 && (
        <div className="bg-cq-milk rounded-cq-lg shadow-cq-card overflow-hidden">
          <div className="bg-cq-alert text-white px-6 py-3">
          ...
          <div className="p-4 space-y-3 max-h-64 overflow-y-auto">
            {criticalAlerts.map(alert => (
              <div key={alert.id} className={`p-4 rounded-cq-md border ${getAlertBgColor(alert.severity)}`}>
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      {alert.detail && (
                        <div className="text-sm text-cq-ink-2 mt-1">{alert.detail}</div>
                      )}
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded text-sm font-medium ${
                        alert.severity === 'critical' 
                          ? 'bg-cq-alert text-white hover:opacity-90'
                          : alert.severity === 'warning'
                          ? 'bg-cq-warn text-white hover:bg-cq-warn'
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
        <div className="bg-cq-milk rounded-cq-lg shadow-cq-card p-4">
          <div className="flex items-center justify-between mb-2">
            <Coffee className="text-cq-ink-3" size={20} />
            {/* removed hardcoded "+12%" — no prev-period comparison */}
          </div>
          <div className="text-2xl font-bold">{systemMetrics.totalOrders}</div>
          <div className="text-xs text-cq-ink-3">Active Orders</div>
        </div>
        
        <div className="bg-cq-milk rounded-cq-lg shadow-cq-card p-4">
          <div className="flex items-center justify-between mb-2">
            <Clock className="text-cq-ink-3" size={20} />
            <span className={`text-xs ${systemMetrics.avgWaitTime > 15 ? 'text-cq-alert' : 'text-cq-ready'}`}>
              {systemMetrics.avgWaitTime > 15 ? '↑' : '↓'}
            </span>
          </div>
          <div className="text-2xl font-bold">{systemMetrics.avgWaitTime}m</div>
          <div className="text-xs text-cq-ink-3">Avg Wait</div>
        </div>
        
        <div className="bg-cq-milk rounded-cq-lg shadow-cq-card p-4">
          <div className="flex items-center justify-between mb-2">
            <Users className="text-cq-ink-3" size={20} />
          </div>
          <div className="text-2xl font-bold">{systemMetrics.activeStations}/{stations.length}</div>
          <div className="text-xs text-cq-ink-3">Active Stations</div>
        </div>
        
        <div className="bg-cq-milk rounded-cq-lg shadow-cq-card p-4">
          <div className="flex items-center justify-between mb-2">
            <Activity className="text-cq-ink-3" size={20} />
          </div>
          <div className="text-2xl font-bold">{systemMetrics.loadPercentage}%</div>
          <div className="text-xs text-cq-ink-3">System Load</div>
          <div className="mt-1 bg-cq-wash h-2 rounded-full overflow-hidden">
            <div 
              className={`h-full ${
                systemMetrics.loadPercentage > 90 ? 'bg-cq-alert' :
                systemMetrics.loadPercentage > 70 ? 'bg-cq-warn' :
                'bg-cq-ready'
              }`}
        ...
        <div className="bg-cq-milk rounded-cq-lg shadow-cq-card p-4">
          <div className="flex items-center justify-between mb-2">
            <TrendingUp className="text-cq-ink-3" size={20} />
          </div>
          <div className="text-2xl font-bold">{systemMetrics.ordersPerHour}</div>
          <div className="text-xs text-cq-ink-3">Orders/Hour</div>
        </div>
        
        <div className="bg-cq-milk rounded-cq-lg shadow-cq-card p-4">
          <div className="flex items-center justify-between mb-2">
            <Target className="text-cq-ink-3" size={20} />
          </div>
          <div className="text-2xl font-bold text-cq-ready">{systemMetrics.customerSatisfaction != null ? `${systemMetrics.customerSatisfaction}%` : '--'}</div>
          <div className="text-xs text-cq-ink-3">Satisfaction</div>
        </div>
        
        <div className="bg-cq-milk rounded-cq-lg shadow-cq-card p-4">
          <div className="flex items-center justify-between mb-2">
            <BarChart2 className="text-cq-ink-3" size={20} />
          </div>
          <div className="text-2xl font-bold">${systemMetrics.totalRevenue}</div>
          <div className="text-xs text-cq-ink-3">Revenue</div>
        </div>
        ...
        <div className="bg-cq-milk rounded-cq-lg shadow-cq-card p-4">
          ...
          <div className="text-xs text-cq-ink-3">Peak Time</div>
        </div>
      </div>

      {/* Station Status Grid with Enhanced Information */}
      <div className="bg-cq-milk rounded-cq-lg shadow-cq-card">
        <div className="px-4 sm:px-6 py-4 border-b border-cq-line flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    station.status === 'active' ? 'bg-cq-ready-wash text-cq-ready' :
                    station.status === 'paused' ? 'bg-cq-warn-wash text-cq-warn' :
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-cq-ink-2">Queue</span>
                    <span className="font-semibold">{total} orders</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-cq-ink-2">In Progress</span>
                    <span className="font-semibold">{inProgress.length}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-cq-ink-2">Wait Time</span>
                    <span className={`font-semibold ${avgWait > 15 ? 'text-cq-alert' : ''}`}>
                      {avgWait}m
                    </span>
                  </div>
                  {station.current_barista && (
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-cq-ink-2">Barista</span>
                ...
                {/* Load indicator bar */}
                <div className="mt-3 bg-cq-wash h-3 rounded-full overflow-hidden">
                  <div 
                    className={`h-full transition-all ${
                      total / (station.max_concurrent_orders || 5) > 1 ? 'bg-cq-alert' :
                      total / (station.max_concurrent_orders || 5) > 0.8 ? 'bg-cq-warn' :
                      total / (station.max_concurrent_orders || 5) > 0.5 ? 'bg-cq-caramel' :
                      'bg-cq-ready'
                    }`}
                ...
                <div className="mt-3 flex flex-wrap gap-1">
                  {station.capabilities?.standard_coffee && (
                    <span className="text-xs bg-cq-wash text-cq-ink-2 px-2 py-1 rounded">Coffee</span>
                  )}
                  {station.capabilities?.alternative_milk && (
                    <span className="text-xs bg-cq-caramel-wash text-cq-caramel-deep px-2 py-1 rounded">Alt Milk</span>
                  )}
                  {station.capabilities?.high_volume && (
                    <span className="text-xs bg-cq-caramel-wash text-cq-caramel-deep px-2 py-1 rounded">High Vol</span>
                  )}
                </div>
              </div>
            );
      ...
      <div className="bg-cq-milk rounded-cq-lg shadow-cq-card p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center p-4 rounded-cq-md transition-all ${
              isPaused 
                ? 'bg-cq-ready hover:bg-cq-ready' 
                : 'bg-cq-warn hover:bg-cq-warn'
            } text-white`}
          >
        ...
        <div className="p-4 space-y-2 max-h-64 overflow-y-auto">
          {recentActivities.length > 0 ? (
            recentActivities.map(activity => (
              <div key={activity.id} className="flex items-center space-x-3 py-2 border-b border-cq-line last:border-0">
                <span className="text-xs text-cq-ink-3 w-16">
                  {new Date(activity.timestamp).toLocaleTimeString()}
                </span>
                <ChevronRight size={14} className="text-cq-ink-3" />
                <span className={`text-sm ${
                  activity.type === 'emergency' ? 'text-cq-alert font-semibold' :
                  activity.type === 'system' ? 'text-cq-caramel-deep' :
                  'text-cq-ink-2'
                }`}>
                  {activity.message}
                </span>
          ...
          <div className="bg-cq-milk rounded-cq-md p-6 max-w-md w-full mx-4">
            ...
            <div className="flex space-x-3">
              <button
                ...
                className="flex-1 bg-cq-alert text-white py-2 rounded-cq-md hover:bg-cq-alert disabled:opacity-50 disabled:cursor-not-allowed font-medium"
              >
              ...
              <button
                ...
                className="flex-1 bg-cq-line text-cq-ink-2 py-2 rounded-cq-md hover:bg-cq-ink-3 font-medium"
              >
                Cancel
              </button>
            </div>
