...
const EnhancedCommunicationHub = () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'delivered': return 'text-cq-ready bg-cq-ready-wash';
      case 'sent': return 'text-cq-caramel-deep bg-cq-caramel-wash';
      case 'sending': return 'text-cq-caramel bg-cq-caramel-wash';
      case 'failed': return 'text-cq-alert bg-cq-alert-wash';
      case 'scheduled': return 'text-cq-caramel-deep bg-cq-caramel-wash';
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center space-x-2 text-sm">
              <Wifi className="w-4 h-4 text-cq-ready" />
              <span className="text-cq-ink-2">All channels online</span>
            </div>
            
            <button 
              onClick={() => setShowBroadcastModal(true)}
              className="px-4 py-2 bg-cq-roast text-white rounded-cq-md hover:bg-cq-caramel-deep flex items-center space-x-2"
            >
        ...
        <div className="flex space-x-1 border-b overflow-x-auto">
          <button
            ...
            className={`px-4 py-2 font-medium border-b-2 transition-colors ${
              activeChannel === 'all'
                ? 'text-cq-roast border-cq-roast-deep'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Conversations List */}
        <div className="lg:col-span-2 bg-cq-milk rounded-cq-lg shadow-cq-card">
          <div className="p-4 border-b">
            <div className="flex items-center space-x-3">
              {/* Search */}
              <div className="relative flex-1">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-cq-ink-3" />
                <input
                  type="text"
                  value={searchTerm}
              ...
              <button className="p-2 hover:bg-cq-wash rounded-cq-md">
                <RefreshCw className="w-5 h-5 text-cq-ink-2" />
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:h-96">
            ...
            <div className="w-full max-h-56 sm:max-h-none sm:w-1/3 border-b sm:border-b-0 sm:border-r overflow-y-auto">
              {conversations.map(conv => (
                <div
                  ...
                  className={`p-4 border-b cursor-pointer hover:bg-cq-wash ${
                    selectedConversation?.id === conv.id ? 'bg-cq-caramel-wash' : ''
                  }`}
                >
                  ...
                  <p className="text-sm text-cq-ink-2">{conv.recipient}</p>
                  <p className="text-sm text-cq-ink-3 truncate">{conv.lastMessage}</p>
                  {conv.unreadCount > 0 && (
                    <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-cq-caramel-wash text-cq-caramel-deep mt-1">
            ...
            <div className="flex-1 flex flex-col">
              {selectedConversation ? (
                <>
                  {/* Header */}
                  <div className="p-4 border-b bg-cq-wash">
                    <h3 className="font-semibold">{selectedConversation.customerName}</h3>
                    <p className="text-sm text-cq-ink-2">{selectedConversation.recipient}</p>
                  </div>
                    ...
                    {selectedConversation.messages.map(msg => (
                      ...
                      >
                        <div className={`max-w-xs ${
                          ...
                          <div className={`p-3 rounded-cq-md ${
                            msg.type === 'outbound'
                  ...
                  <div className="p-4 border-t">
                    <div className="flex space-x-2">
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-cq-roast text-white rounded-cq-md hover:bg-cq-caramel-deep disabled:opacity-50"
                      >
                ...
                <div className="flex-1 flex items-center justify-center text-cq-ink-3">
                  <div className="text-center">
                    <MessageSquare className="w-12 h-12 mx-auto mb-2 text-cq-line" />
                ...
                .map(template => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      ...
                      {template.costly && (
                        <span className="text-xs text-cq-caramel-deep bg-cq-caramel-wash rounded px-1.5 py-0.5 whitespace-nowrap"
                              title="Contains a character outside the plain SMS set (usually an emoji or a dash). That doubles the cost of every send.">
                          costs double
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-cq-ink-2 mb-2">
                      {template.unset
                        ? <em className="text-cq-ink-3">Not set — the built-in wording is used</em>
                        : template.template}
                    </p>
      ...
      <div className="bg-cq-milk rounded-cq-lg shadow-cq-card p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="text-center p-4 bg-cq-caramel-wash rounded-cq-md">
            <p className="text-3xl font-bold text-cq-caramel-deep">
              {messages.filter(m => m.timestamp > new Date(Date.now() - 24*60*60*1000)).length}
            </p>
            <p className="text-sm text-cq-ink-2">Messages Today</p>
            {/* removed hardcoded "+12% vs yesterday" — no day-over-day
                comparison is computed. */}
          </div>
          
          <div className="text-center p-4 bg-cq-ready-wash rounded-cq-md">
            <p className="text-3xl font-bold text-cq-ready">
              {messages.length === 0
                ? '\u2014'
                : `${((messages.filter(m => m.status === 'delivered').length / messages.length) * 100).toFixed(1)}%`}
            </p>
            <p className="text-sm text-cq-ink-2">Delivery Rate</p>
          ...
          <div className="text-center p-4 bg-cq-caramel-wash rounded-cq-md">
            <p className="text-3xl font-bold text-cq-caramel">—</p>
            <p className="text-sm text-cq-ink-2">Avg Response Time</p>
            <p className="text-xs text-cq-ink-3 mt-1">not yet measured</p>
          </div>

          <div className="text-center p-4 bg-cq-caramel-wash rounded-cq-md">
            <p className="text-3xl font-bold text-cq-caramel-deep">—</p>
            <p className="text-sm text-cq-ink-2">Satisfaction Score</p>
            <p className="text-xs text-cq-ink-3 mt-1">no feedback pipeline yet</p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-cq-milk rounded-cq-lg shadow-cq-card-xl max-w-md w-full p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => { setShowBroadcastModal(false); setBcResult(null); }}
                      className="text-cq-ink-3 hover:text-cq-ink-2">✕</button>
            </div>
            <label className="block text-sm text-cq-ink-2 mb-1">Audience</label>
            <select
              value={bcAudience}
              onChange={e => setBcAudience(e.target.value)}
              className="w-full px-3 py-2 border border-cq-line rounded mb-3"
            >
            ...
            <label className="block text-sm text-cq-ink-2 mb-1">Message</label>
            <textarea
              value={bcMessage}
              ...
              className="w-full px-3 py-2 border border-cq-line rounded mb-1"
            />
            <p className="text-xs text-cq-ink-3 mb-3">{bcMessage.length}/480 — capped at 500 recipients per send.</p>
            {bcResult && (
              <div className={`mb-3 text-sm px-3 py-2 rounded ${bcResult.ok ? 'bg-cq-ready-wash text-cq-ready' : 'bg-cq-alert-wash text-cq-alert'}`}>
                {bcResult.msg}
              </div>
            )}
            <div className="flex items-center justify-end gap-2">
              <button
                ...
                disabled={bcSending}
                className="px-4 py-2 rounded border border-cq-line hover:bg-cq-wash text-cq-ink-2 disabled:opacity-50"
              >
                Preview recipients
              </button>
              <button
                ...
                className="px-4 py-2 rounded bg-cq-roast hover:bg-cq-caramel-deep text-white disabled:opacity-50 flex items-center gap-2"
              >
