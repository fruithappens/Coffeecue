...
const StationCapabilityCard = ({ station, choices, onSaved }) => {
  ...
  if (caps == null) {
    return (
      <div className="border rounded-cq-md p-4 bg-cq-wash">
  ...
  const RowCheckboxes = ({ label, items, selected, fieldKey }) => (
    <div>
      <div className="text-xs font-semibold text-cq-ink-3 uppercase tracking-wide mb-1">{label}</div>
  ...
  return (
    <div className={`border rounded-cq-md p-4 ${dirty ? 'border-cq-caramel bg-cq-caramel-wash' : 'bg-cq-milk'}`}>
      <div className="flex justify-between items-start mb-3">
        <div>
          <h4 className="font-bold text-lg">{station.name || `Station ${station.id}`}</h4>
          {station.location && (
            <div className="text-xs text-cq-ink-3">{station.location}</div>
          )}
        </div>
        <div className="flex items-center gap-2">
          {status === 'saved' && (
            <span className="text-cq-ready text-sm flex items-center">
              <Check size={14} className="mr-1" /> Saved
            </span>
          )}
          {status === 'error' && (
            <span className="text-cq-alert text-sm flex items-center">
          ...
          {dirty && (
            <button
              onClick={revert}
              className="text-xs px-2 py-1 bg-cq-line hover:bg-cq-line rounded inline-flex items-center"
          ...
          <button
            ...
            className="text-xs px-3 py-1 bg-cq-caramel hover:bg-cq-caramel-deep text-white rounded disabled:opacity-40 inline-flex items-center"
          >
      ...
      <div className="space-y-3">
        ...
        {selectedDrinks.size === 0 && (
          <div className="text-xs text-cq-ink-3 italic -mt-2">
        ...
        <div className="flex flex-wrap items-center gap-4 pt-2 border-t border-cq-line">
          ...
          {altMilkContradiction && (
            <span className={`text-xs px-2 py-0.5 rounded ${current.alt_milk ? 'bg-cq-alert-wash text-cq-alert' : 'bg-cq-caramel-wash text-cq-caramel-deep'}`}>
              {current.alt_milk
          ...
          <label className="inline-flex items-center text-sm" title="Max orders this station can hold in its queue (used by load balancing)">
            <span className="mr-2 text-cq-ink-2">Max queue:</span>
            <input
              type="number"
              min="1"
              max="100"
              ...
              className="w-16 px-2 py-1 border border-cq-line rounded text-sm"
            />
          </label>
          <label className="inline-flex items-center text-sm" title="How many drinks this station makes at the same time (steam wands / group heads / baristas). Feeds the wait-time estimate — a 3-group station clears its queue ~3× faster.">
            <span className="mr-2 text-cq-ink-2">Drinks at once:</span>
            <input
              type="number"
              min="1"
              max="20"
          ...
          <label className="inline-flex items-center text-sm" title="How many baristas are on this station (context for throughput; shown in the post-event report).">
            <span className="mr-2 text-cq-ink-2">Baristas:</span>
            <input
              type="number"
              min="0"
              max="20"
          ...
          <label className="inline-flex items-center text-sm" title="The team's expected throughput in orders/hour. Used for the wait estimate before real make-times exist; the post-event report shows the ACTUAL rate so you can refine this next time.">
            <span className="mr-2 text-cq-ink-2">Expected /hour:</span>
            <input
              type="number"
              min="0"
              max="1000"
              ...
              className="w-20 px-2 py-1 border border-cq-line rounded text-sm"
            />
          </label>
        </div>
      </div>
...
const StationCapabilitiesEditor = () => {
  ...
  if (loading) {
    return <div className="p-4 text-sm text-cq-ink-3">Loading stations and inventory…</div>;
  }

  if (!stations.length) {
    return (
      <div className="p-4 text-sm text-cq-ink-2">
