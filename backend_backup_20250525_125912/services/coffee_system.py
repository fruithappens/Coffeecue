...
class CoffeeOrderSystem:
    ...
    def _handle_commands(self, phone, message, state):
        ...
        elif message_upper == 'OPTIONS' or message_upper == 'COMMANDS':
            return self._handle_options_command()
        
        # Check for MENU command
        elif message_upper == 'MENU':
    ...
    def _handle_help_command(self):
        ...
        return (
            ...
            "- STATUS: Check your order status and get tracking link\n"
    ...
    def _handle_options_command(self):
        ...
        return (
            ...
            "- CANCEL: Cancel your pending order\n"
            "- MENU: See available coffee options\n"
            "- INFO: Get ordering instructions\n" 
            "- OPTIONS: See this list of commands\n"
            "- VIP: Enter VIP code (if applicable)\n"
            "- USUAL: Order your usual coffee"
        )
    ...
    def _handle_awaiting_name(self, phone, message, state):
        ...
        self._set_conversation_state(phone, 'awaiting_coffee_type', {'name': name})
        return f"Nice to meet you, {name}! What type of coffee would you like?"
    ...
    def _handle_awaiting_confirmation(self, phone, message, state):
        ...
        if message_upper == 'YES' or message_upper == 'Y':
            ...
            # Add option to order for a friend and set up state to handle the response
            ...
            return (
                f"{order_response}\n\n"
                f"Would you like to order another coffee for a friend? (Reply FRIEND to add another order, or NO to finish)"
            )
    ...
    def _handle_awaiting_friend_decision(self, phone, message, state):
        ...
        if message_upper == 'NO' or message_upper == 'N' or message_upper == 'FINISH' or message_upper == 'DONE' or message_upper == 'END':
            ...
            self._set_conversation_state(phone, 'completed')
            if total_orders > 1:
                return f"Thanks, {primary_name}! Your group order of {total_orders} coffees has been confirmed. They will all be ready for pickup at Station {station_id}."
            else:
                return f"Thanks, {primary_name}! Your order has been confirmed. It will be ready for pickup at Station {station_id}."
    ...
    def _handle_awaiting_friend_confirmation(self, phone, message, state):
        ...
        if message_upper == 'YES' or message_upper == 'Y':
            # Confirm the order for the friend
            ...
            return (
                f"{order_response}\n\n"
                f"Would you like to order for another friend? (Reply FRIEND to add another order, or NO to finish)"
            )
            ...
            total_orders = len(group_orders) + 1  # +1 for the primary order
            self._set_conversation_state(phone, 'completed')
            return f"Thanks, {primary_name}! Your group order of {total_orders} coffees has been confirmed. They will all be ready for pickup at the same station."
    ...
    def _confirm_order(self, phone, order_details, name):
        ...
        try:
            ...
            # Step 2: Update customer preferences in a separate transaction
            try:
                # Check if customer exists
                if db_type == "sqlite":
    ...
    def _assign_station(self, is_vip=False, milk_type=None):
        ...
        try:
            ...
            if not stations:
                ...
                logger.warning("No stations found, defaulting to station 1")
                return 1, False
            ...
            # Check if the requested milk type is a special case
            is_alt_milk = milk_type and milk_type.lower() in ['soy milk', 'almond milk', 'oat milk']
            ...
            # Special handling for alternative milk orders
            if is_alt_milk:
                alt_milk_stations = [s for s in active_stations if s['alt_milk_available']]
                if alt_milk_stations:
                    alt_milk_stations.sort(key=lambda s: s['load'])
    ...
    def create_walk_in_order(self, order_data):
        ...
        try:
            ...
            # Assign to a station
            station_id = order_data.get('station_id', None) or self._assign_station(order_data.get('vip', False))
