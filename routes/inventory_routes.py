...
# Get event-level inventory
@bp.route('/api/inventory/event', methods=['GET'])
...
def get_event_inventory():
    ...
    try:
        ...
        # Get aggregated inventory across all stations
        cursor.execute("""
            SELECT 
                category,
                name,
                ...
                unit,
                MIN(minimum_threshold) as min_threshold
        ...
        for item in items:
            ...
            inventory[category].append({
                ...
                'unit': item['unit'],
                'minThreshold': float(item['min_threshold']) if item['min_threshold'] else 0
            })
        ...
        logger.error(f"Error fetching event inventory: {str(e)}")
        return jsonify({
            'status': 'error',
