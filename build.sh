...
cd "$(dirname "$0")/Barista Front End" && DISABLE_ESLINT_PLUGIN=true CI=false GENERATE_SOURCEMAP=false npm run build 2>&1 | grep -E "Compiled|Failed|main\.[a-f0-9]+\.js" | tee /tmp/cupq_build.log; grep -q "Failed" /tmp/cupq_build.log && { echo "BUILD FAILED -- nothing copied"; exit 1; }
