...
if CORS_ALLOWED_ORIGINS_ENV:
    ...
    # Add Railway domain patterns if not already specified
    railway_patterns = ['.railway.app', '.railway.app:*']
    has_railway = any('railway.app' in origin for origin in CORS_ALLOWED_ORIGINS)
    
    if not has_railway and any(pattern in str(CORS_ALLOWED_ORIGINS) for pattern in ['localhost', '127.0.0.1']):
        # Development mode detected, add Railway production domain placeholder
